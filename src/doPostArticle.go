...
//------------------------------------------------------------------------
func doPostArticle(w http.ResponseWriter, r *http.Request) {
	...
	sql := "insert into post (userid, time, title, content, replyTo) values (?, now(), ?, ?, 0)"
	stmt, err := dbConn.Prepare(sql)
	rec, err := stmt.Exec(uid, title, content)
